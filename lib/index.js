...
module.exports=function specifyImport({types:t}){return{visitor:{ImportDeclaration(path,state){let sourceDir=path.node.source.value;// if not a relative path, ignore
...
let modules=state.moduleList;for(let i=0;i<modules.length;++i){let internalModName=`selectedModule${i}`;// add import to obj
...
path.insertAfter(t.importDeclaration([t.importDefaultSpecifier(t.identifier(internalModName))],t.stringLiteral(pathName+modules[i])))}}}}};